# Audit Portfolio

- [About Me](#about-me)
- [Competitive Audits](#competitive-audits)
- [Private Audits](#private-audits)

# About Me

I am Ritik Agarwal smart contract security researcher focused on DeFi. My findings have been validated across contests on [Cantina](https://cantina.xyz/), [Sherlock](https://audits.sherlock.xyz/) including High-severity issues in lending, vault, AMM, and order-book protocols.

**Proficient in:** Solidity · Foundry · DeFi protocol mechanics

For private audit or consulting requests, please reach out via:

- X / Twitter: [@RitikkAgarwalll](https://x.com/RitikkAgarwalll)

# Competitive Audits

| Contest | Description | Result | Report |
|---|---|---|---|
| `Kuru ~ Cantina` | On-chain central limit order book exchange with vaults and margin trading. | 🥉 3rd | [4 findings →](./Kuru/readme.md) |
| `Ammplify ~ Sherlock` | Uniswap v4-based liquidity amplification protocol with maker/keeper hooks. | 19th | [1 finding →](./Ammplify/README.md) |
| `Jigsaw ~ Cantina` | Lending protocol with collateralized positions, debt, and liquidations. | 92th | [2 findings →](./Jigsaw-contracts/readme.md) |
| `Mighty Finance ~ Cantina` | Leveraged yield farming and lending protocol on Sonic. | 117th | [5 findings →](./Mighty_Finance/readme.md) |
| `Alchemix V3 ~ Cantina` | Loans backed by future yield — collateral earns interest that repays debt. | 86th | [2 findings →](./Alchemix/readme.md) |
| `Gamma Protocol ~ CodeHawks` | Active liquidity management protocol — GMX perpetual vault. | — | [1 finding →](./Gamma_Liquidity_Management/README.md) |
| `QuantAMM ~ CodeHawks` | AMM with weight-based liquidity management and configurable fees. | — | [1 finding →](./QuantAMM/readme.md) |
| `Limit Break AMM ~ Guardian` | AMM with customizable token swap hooks. | — | [2 findings →](./Limit_Break_AMM/README.md) |

# Private Audits

Due to NDA agreements, details of private engagements are kept confidential and reports are not disclosed publicly.

| Client | Description | Ecosystem | Report |
|---|---|---|---|
| Confidential (NDA) | Private security audit of a distributed ledger (DLT) platform built on Go. | DLT · Go | 🔒 Not disclosed (NDA) |
